...
char create_cse_base(CSEBaseStruct * csebase, char isTableCreated) {
    ...
    if (isTableCreated == FALSE) {
        ...
        const char *createTableSQL = "CREATE TABLE IF NOT EXISTS mtc (  ty INTEGER,  ri TEXT PRIMARY KEY,  rn TEXT,  pi TEXT,  aei TEXT,  csi TEXT,  cst INTEGER,  api TEXT,  rr TEXT,  et DATETIME,  ct DATETIME,  lt DATETIME,  url TEXT,  lbl TEXT,  acpi TEXT,  daci TEXT,  poa TEXT,  srt TEXT,  blob TEXT,  cbs INTEGER,  cni INTEGER,  mbs INTEGER,  mni INTEGER,  st INTEGER,  cnf TEXT,  cs INTEGER,  con INTEGER, FOREIGN KEY(pi) REFERENCES mtc(ri) ON DELETE CASCADE);";

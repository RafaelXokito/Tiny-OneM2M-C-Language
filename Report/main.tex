...
\newacronym{ccas}{CCAS}{Cluster-based Congestion-miti\-gating Access Scheme}
...
\newacronym{cse}{CSE\_Base}{Common Services Entity}
...
For \gls{aes}, \gls{cnts}, \gls{subs}, users can perform different actions on the resource with the corresponding \gls{http} requests. These requests are sent to a specific URL structure, with a specific payload specified by the OneM2M standard. The \gls{cin} resource only allows Create, Read, and Delete operations.

The system also provides functionality to retrieve the \gls{cse} (root) resource information with a specific GET request. It's allowed to perform a discovery operation using GET request with query parameters to filter resources based on their type, labels, and many other attributes. These operations can be used to retrieve general or specific information about the resources available in the system.
...
In this setup, there is one main table named \texttt{mtc}, which is represented in Table \ref{tab:database}. It is a combination of multiple entity types in the TinyOneM2M hierarchy, such as \gls{cse}, \gls{ae}, \gls{cnt}, \gls{cin}, and \gls{sub}. The attributes of these entities are represented as columns in the \texttt{mtc} table using their abbreviations.

Looking deeper into the table structure, the \texttt{ri} column serves as the primary key, providing unique identification for each record. It establishes relations with the \texttt{pi} column, which refers to the \texttt{ri} of the parent record. This sets up a hierarchical relationship between records addressing the resource hierarchy from OneM2M standard.
...
Furthermore, another proposed area for future work is to conduct tests utilizing the various scenarios outlined in Section \ref{architecture}.

...
Our system architecture consists of a client-server model, where clients interact with our server over the internet. The server acts as an intermediary between the clients and the database, providing resource availability and implementing CRUD operations for the Application Entity (AE) resource and Read operations for Common Service Entities (CSE). Figure \cite{fig:architecture} provides a visual representation of our architecture.

\begin{figure}[t]
	\centering
	\includegraphics[width=7cm]{Architecture.jpg}
	\caption{Architecture of the Proposed Solution: A Client-Server Model for Resource Management in the oneM2M Standard}
	\label{fig:architecture}
\end{figure}

At the core of our system, the server acts as the central point of communication, facilitating the exchange of data and requests between the clients and the database. This approach enables us to provide a reliable and secure service, ensuring that client requests are handled efficiently and that their data is safeguarded against unauthorized access or manipulation.

By focusing on the AE and CSE resources, we can provide a targeted and streamlined solution that caters specifically to the needs of our clients. This approach allows us to deliver a service that is efficient, scalable, and cost-effective, enabling us to effectively meet the demands of our users and provide value to our clients.

In summary, our architecture provides a robust and secure system that effectively facilitates communication between clients and the database, enabling us to provide a targeted and streamlined service that caters to the needs of our users.

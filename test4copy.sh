...
while [ $count -lt 2000 ]; do
    # POST
    time=$(curl -X POST "http://10.20.246.135:10000/onem2m" -H "Content-Type: application/vnd.onem2m-res+json" -d "{\"m2m:ae\": {\"api\": \"placeholder2\",\"rr\": \"true\",\"rn\": \"AE-A${count}\",\"et\": \"20230630T234737\",\"lbl\": [\"interropetores\",\"xpto2\"],\"poa\": [\"http://127.0.0.1:1234\"],\"acpi\": [\"/id-in/acpCreateACPs\"]}}" -w "%{time_total}" -o /dev/null)

...
while [ $count -lt 2000 ]; do
    # GET
    time=$(curl -X GET "http://10.20.246.135:6000/onem2m/AE-"$count -w "%{time_total}" -o /dev/null)
    echo "$time" >> "$get_times"
...
while [ $count -lt 2000 ]; do
    # DELETE
    time=$(curl -X DELETE "http://10.20.246.135:6000/onem2m/AE-"$count -w "%{time_total}" -o /dev/null)
